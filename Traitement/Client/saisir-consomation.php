...
$client = getClientByUserId($_SESSION['user_id']);
if (!$client) {
    die("Erreur : Vous devez avoir un profil client pour saisir une consommation");
}
...
// Récupérer la dernière consommation si elle existe
if (!empty($consommations)) {
    $last_consumption = $consommations[0];
}
